...
- Live follow-up: minimizing/background playback caused intermittent audio stalls when page clock callbacks were throttled. Added a pure clock-extrapolation helper and offscreen fallback that advances only the AudioWorklet master clock and CMAF prefetch from the last real video clock while playback remains active; actual page clocks override it immediately on recovery. CDP hidden-visibility smoke kept `active`, advanced video/audio time, and did not increase underruns.
- Live follow-up: residual background stutter remained with a one-second CMAF PCM budget and two-second prefetch threshold. Raised only the CMAF decode cushion to 3 seconds and started the next bounded fragment window with 4 seconds of compressed-media lead; local raw playback remains on its original 1-second budget.
- Live follow-up: background output still showed occasional short stalls despite the larger queue. Configured the offscreen AudioContext with `latencyHint=playback` and periodically resumes it if an active, unpaused session observes a non-running context; the hidden-window CDP run stayed active with a roughly 3-second PCM cushion.
- Live follow-up: the remaining minimized-window stutter was caused by the hidden page's coarse clock updates making the Worklet repeatedly alternate between `wait` and `trim/resync`. Playing hidden pages now suppress non-authoritative timer clocks, while the Worklet advances its master media clock once per rendered quantum and accepts forced clocks for pause, seek, buffering, visibility, and recovery. CDP minimized-window verification stopped resync growth, and the user confirmed the audio no longer stutters.
- Live follow-up: pausing a healthy CMAF session for longer than the 10-second decoder-progress window could falsely fail the session while backpressure was intentionally waiting. The progress watchdog now freezes during pause/buffering and resumes with the remaining active-time budget; a new pure-core regression test covers long pauses without weakening the active decode timeout.
