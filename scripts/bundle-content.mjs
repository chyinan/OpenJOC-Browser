...
export function createContentBundle(extensionDirectory) {
  ...
  return [
    ...
    `const __openjocOverlayState = (() => { const {normalizeOutputGainDb} = __openjocOutputGain; const {DEFAULT_OVERLAY_LANGUAGE, normalizeOverlayLanguage} = __openjocOverlayI18n; ${state} return {advanceOverlayState, createOverlayState, needsOverlayMarkupRebuild, overlayRendererLabel, resetOverlayState, OVERLAY_RENDERER_OPTIONS}; })();`,
    ...
    `const __openjocOverlayController = (() => { const {normalizeOutputGainDb, OUTPUT_GAIN_MIN_DB, OUTPUT_GAIN_MAX_DB, OUTPUT_GAIN_STEP_DB} = __openjocOutputGain; const {OVERLAY_LANGUAGE_OPTIONS, isOverlayLanguage, normalizeOverlayLanguage, overlayMessage} = __openjocOverlayI18n; const {advanceOverlayState, createOverlayState, needsOverlayMarkupRebuild, overlayRendererLabel, resetOverlayState, OVERLAY_RENDERER_OPTIONS} = __openjocOverlayState; ${controller} return {createJocOverlayController}; })();`,
    `(() => { const {normalizeOutputGainDb} = __openjocOutputGain; const {normalizeOverlayLanguage} = __openjocOverlayI18n; const {isRuntimeMessage} = __openjocExtensionProtocol; const {createJocOverlayController} = __openjocOverlayController; const {acknowledgeStart, createStartHandshake, nextStartHandshakeAction, recordStartAttempt, shouldAcceptPlaybackStatus, shouldDispatchSessionRecovery, shouldExpirePlaybackStatus, shouldRestartAfterSeek} = __openjocStartHandshake; ${content} })();`,
  ].join('\n\n');
}
